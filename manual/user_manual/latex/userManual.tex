...
To perform a grid removal operation, select the \emph{Remove Grid} option from the \emph{Axes} menu. A new set of controls should appear on the side with the following:
\begin{itemize}
...
\item{{\bf Edit Labels: }This option is only available for Bar Charts. After clicking this button, you can edit the labels for a data points by clicking on them.}
\end{enumerate}
