...
QUnit.test("Log base 2 axes", function (assert) {
    ...
    // Given linearly aligned axes
    let calib = new wpd.Calibration(2);    
    calib.addPoint(0, 99, Math.pow(2,-5).toString(), Math.pow(2,-20).toString());  // X1 = 2^-5 at (0, 99)px
    calib.addPoint(99, 99, Math.pow(2,12).toString(), Math.pow(2,-20).toString()); // X2 = 2^12 at (99, 99)px
    calib.addPoint(0, 99, Math.pow(2,-5).toString(), Math.pow(2,-20).toString());  // Y1 = 2^-20 at (0, 99)px
